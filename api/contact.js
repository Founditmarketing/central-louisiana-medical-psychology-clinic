...
module.exports = async (req, res) => {
  ...
  const to   = process.env.CONTACT_TO_EMAIL   || "NewPatient@centrallamedpsych.com";
  const from = process.env.CONTACT_FROM_EMAIL || "Central LA Medical Psychology Clinic <leads@clmpc.com>";

  const subject = `New lead from clmpc.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on clmpc.com`;
  const html = `
<h2>New lead from the Central Louisiana Medical Psychology Clinic site</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on clmpc.com</p>`;
